...
bit_file* bit_open(int *data, int *size, int mode, UINTMAX_T buff_size) {
...
int bit_read(bit_file* bfp, char* buff_out, UINTMAX_T n_bits, uint8_t ofs) {
    ...
    while (n_bits > 0) {
        ...
        if (w_len == 0) {
	  //printf("DATA=%d FILEPOS=%d\n",bfp->data[(*(bfp->fpos))],(*(bfp->fpos)));
	  
	    //c = bfp->data[(*(bfp->fpos))++];
	    bfp->buff[0]=bfp->data[(*(bfp->fpos))++];
	    c=1;
	    //printf("BIT_READ c=%d\n", c);
