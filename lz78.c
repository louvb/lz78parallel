...
ht_dictionary* ht_dictionary_new(uint32_t d_size) {
    ...
    if (dict == NULL){
        printf("P%d ERROR ALLOCATING MEMORY ht_dictionary_new\n", myId);
        return NULL;
    }
...
dictionary* dictionary_new(uint32_t d_size) {
    ...
    if (dict == NULL){
        printf("P%dERROR ALLOCATING MEMORY dictionary_new\n", myId);
        return NULL;
    }
...
int decompress_code(lz78_d* o, uint32_t code) {
    ...
    switch(code) {
    case DICT_CODE_EOF:
      debug3("P%d DICT_CODE_EOF\n",myId);
            o->completed = 1;
            return 0;
    case DICT_CODE_START:
        case DICT_CODE_SIZE:
	  debug3("P%d DICT_CODE_START\n",myId);
            d_main->d_next = DICT_SIZE_MAX;
            o->n_bits = 0;
            return 0;
    default:
            /* Initial operations */
	    debug3("P%d DICT_CODE %d\n",myId, code);
            if (d_main->d_next == DICT_SIZE_MAX) {
                dictionary_destroy(d_main);
                d_main = dictionary_new(code);
		debug3("P%d DICT NEW %d %d\n",myId, o->main->n_bytes, d_main->n_bytes);
                o->main = d_main;
                if (d_main == NULL)
                    return -1;
		debug3("P%d CONTROL 1\n", myId);
    ...
    //printf("DICTUPDATE %d %d %d\n", d_main->d_next, d_main->d_thr, d_main->n_bytes);
    /* Update of secondary if threshold is reached */
    if (d_main->d_next > d_main->d_thr) {
      debug3("P%d CONTROL 3\n", myId);
...
/*FUNCION PRINCIPAL 1*/
uint8_t lz78_compress(lz78_instance* lz78,  int* string_in, int len_in, int *string_out, int *size_out) {
    bit_file* out;
    lz78_c* o;
    int bits;
    int c_in;
    ...
    out = bit_open(string_out, size_out, ACCESS_WRITE, B_SIZE_DEFAULT);
    if (out == NULL)
    ...
    for (;;) {
        ...
        if (o->n_bits > 0) {
	  //debug2("P%d WRITING[%d] %d %d\n", myId, *size_out, o->n_bits, ((char*) &o->bitbuf)[0]);
	...
	//debug2("P%d READING\n", myId);
	    ...
	    c_in=string_in[pos_in];
	    pos_in++;

	}

	//debug2("P%d COMPRESING[%d] %d\n", myId, pos_in, c_in);
        compress_byte(o, c_in);
	//debug2("P%d COMPRESSED[%d] %d\n", myId, pos_in, c_in);
        if (o->completed == 1) {
	  //debug2("P%d CLOSING AND FINISH\n", myId);
            bit_close(out);
            return LZ78_SUCCESS;
        }
    }
...
uint8_t lz78_decompress(lz78_instance* lz78, int* string_in, int len_in, int* string_out, int* len_out) {
    bit_file* in;
    //FILE* out;
    lz78_d* o;
    dictionary* d_main;
    ...
    in = bit_open(string_in, &len_in, ACCESS_READ, B_SIZE_DEFAULT);
    if (in == NULL)
        return LZ78_ERROR_READ;
...
//printf("P%d lzInit\n", myId);
    ...
    for (;;) {
        ...
        if (d_main->n_bytes) {
	      ...
	      debug3("P%d WRITING %d %d\n",myId, written, d_main->n_bytes);
 
	        for(i=0; i<d_main->n_bytes - written; i++)
		  string_out[*len_out+i]=d_main->bytebuf[d_main->offset+written+i];
		ret=d_main->n_bytes - written;
		*len_out+=ret;
		debug3("WRITED %d\n",ret);
                if (ret == -1) {
	    ...
	    debug3("P%d READED %d %s -> %d\n", myId, bits, bits==ret?"OK":"ER", o->bitbuf);
            if (ret == -1)
	    {debug3("P%d ERR reading\n",myId);
                return LZ78_ERROR_READ;}
        ...
        debug3("P%d DECOMPRESING CODE=%d %d\n",myId, o->bitbuf, d_main->n_bytes);
        ret = decompress_code(o, o->bitbuf);
	debug3("P%d DECOMPRESSED CODE=%d %d\n",myId, o->bitbuf, d_main->n_bytes);
        if (ret < 0) {
	  debug3("P%d ERROR No decompressed\n", myId);
            switch(ret) {
                case -1:
